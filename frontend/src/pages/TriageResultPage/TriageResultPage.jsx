...
export default function TriageResultPage({
  ...
  const chatHistory = activeResult.chatHistory || [
    {
      id: "submitted-symptoms",
      role: "owner",
      content:
        activeResult.symptoms || "Symptom description from the latest chat.",
    },
    {
      id: "triage-response",
      role: "assistant",
      content: activeResult.reasoning,
    },
  ];
    ...
    <main className="triage-result-page">
      ...
      <div className="result-layout">
        ...
        <aside className="result-sidebar" aria-label="Screening context">
          <section className="result-context chat-history-card">
            <p className="eyebrow">Chat History</p>
            <h2>Conversation transcript</h2>
            <div className="report-chat-history chat-transcript" aria-label="Chat history">
              {chatHistory.map((message) => (
                <article
                  className={
                    message.role === "owner"
                      ? "report-chat-message report-chat-message--owner"
                      : "report-chat-message report-chat-message--assistant"
                  }
                  key={message.id}
                >
